...
install_netbird() {
    ...
    pkg)
        ...
        # Install the CLI daemon via the official signed .pkg
        install_pkg

        # On macOS desktops, also install the signed GUI app bundle
        if ! $SKIP_UI_APP; then
            echo "Installing NetBird UI for macOS desktop..."
            download_release_binary "$UI_APP"
        fi
    ;;
    brew)
...
is_desktop() {
    ...
    if [ "$(uname)" = "Linux" ]; then
        ...
        # Check if active X11 unix sockets exist
        if [ -d /tmp/.X11-unix ] && ls /tmp/.X11-unix/X* >/dev/null 2>&1; then
            return 0
        fi
        # Check if common window managers or desktop environments are running
...
if [ -z "$SKIP_UI_APP" ]; then
    if is_desktop; then
        SKIP_UI_APP=false
        echo "Auto-detected Desktop environment. NetBird UI will be installed."
    else
        SKIP_UI_APP=true
        echo "Auto-detected Server/Headless environment. NetBird UI installation will be omitted."
    fi
else
    if $SKIP_UI_APP; then
      ...
      echo "SKIP_UI_APP has been set to false in the environment"
      echo "NetBird UI will be installed based on your preference"
    fi
fi
...
if type uname >/dev/null 2>&1; then
	case "$(uname)" in
        Linux)
          ...
          if echo "$UNAME_OUTPUT" | grep -qi "synology"; then
            ...
            if [ -f /etc/os-release ]; then
              ...
              if [ "$SKIP_UI_APP" = "true" ]; then
                  echo "NetBird UI installation will be omitted based on server/headless detection or preference"
              fi
		...
		Darwin)
            ...
            # Check the availability of a compatible package manager
            # If SKIP_UI_APP is true, we fallback to bin package manager to perform CLI-only install
            if check_use_bin_variable || [ "$SKIP_UI_APP" = "true" ]; then
                PACKAGE_MANAGER="bin"
            else
              PACKAGE_MANAGER="pkg"
            fi
